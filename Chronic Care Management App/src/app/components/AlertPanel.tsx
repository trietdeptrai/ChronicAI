        ...
        {unreadAlerts.length > 0 && (
          <>
            ...
            {unreadAlerts.map(alert => (
              <div
                key={alert.id}
                className={`p-3 rounded-xl border backdrop-blur-sm ${getSeverityColor(alert.severity)}`}
              >
        ...
        {readAlerts.length > 0 && (
          <>
            ...
            {readAlerts.map(alert => (
              <div
                key={alert.id}
                className="p-3 rounded-xl border border-gray-200/50 bg-gray-50/50 backdrop-blur-sm opacity-60"
              >
                <div className="flex items-start gap-3">
                  {getSeverityIcon(alert.severity)}
                  <div className="flex-1 min-w-0">
