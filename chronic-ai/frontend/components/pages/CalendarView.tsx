...
export function CalendarView({ onOpenPatientEHR }: CalendarViewProps) {
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {selectedDateAppointments.length > 0 ? (
              selectedDateAppointments.map(appointment => {
                const isNewPatient = appointment.isNewPatient;
                const canOpenEHR = !isNewPatient && appointment.patientId !== 'new';
                
                return (
                  <div
                    key={appointment.id}
                    className={`bg-[rgba(255,255,255,0.6)] rounded-[16px] p-4 border border-[rgba(255,255,255,0.4)] transition-all ${
                      canOpenEHR ? 'hover:shadow-[0px_4px_12px_0px_rgba(0,0,0,0.06)] cursor-pointer' : ''
                    }`}
                    onClick={() => {
                      if (canOpenEHR) {
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {isNewPatient ? (
                            <UserPlus className="w-4 h-4 text-[#4a9fd8]\" />
                          ) : (
                        ...
                        {isNewPatient && (
                          <span className="text-xs px-2 py-0.5 bg-[#4a9fd8] text-white rounded-full font-medium\">
                            Bệnh nhân mới
                          </span>
                        )}
                      </div>
                      ...
                      {appointment.contactPhone && isNewPatient && (
                        <div className="flex items-center gap-2">
                          <Phone className="w-4 h-4 text-[#4a5565]\" />
                          <span className="text-sm text-[#4a5565]\">{appointment.contactPhone}</span>
                        </div>
                      )}
                      ...
                      {canOpenEHR && (
                        <div className="mt-3 pt-3 border-t border-[rgba(229,231,235,0.3)]\">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (onOpenPatientEHR && appointment.patientId !== 'new') {
                                onOpenPatientEHR(appointment.patientId);
                              }
                            }}
                            className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-b from-[#4a9fd8] to-[#2d88c4] text-white rounded-[14px] text-sm font-medium shadow-[0px_4px_6px_0px_rgba(0,0,0,0.1)] hover:shadow-lg transition-all"
                          >
                            <MessageSquare className="w-4 h-4" />
                            <span>Xem hồ sơ & Chat AI</span>
                          </button>
                        </div>
                      )}
                    </div>
