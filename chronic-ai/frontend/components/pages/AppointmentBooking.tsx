        ...
        <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-4 border border-white/40">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={prevWeek} className="p-2 hover:bg-white/40 rounded-xl transition-colors">
                <ChevronLeft className="w-4 h-4 text-gray-600" />
              </button>
              <span className="text-sm text-gray-600 min-w-[120px] text-center">
                {format(weekDays[0], 'd MMM', { locale: vi })} - {format(weekDays[6], 'd MMM', { locale: vi })}
              </span>
              <button onClick={nextWeek} className="p-2 hover:bg-white/40 rounded-xl transition-colors">
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-2xl text-center transition-all ${
                    isSelected ? 'bg-gradient-to-br from-[#4A9FD8] to-[#2D88C4] text-white shadow-lg' :
                    isPast ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white/40 bg-white/60'
                  }`}
                >
                  <div className="text-xs text-gray-600 mb-1">{format(day, 'EEE', { locale: vi })}</div>
                  <div className={`text-lg font-semibold ${isSelected ? 'text-white' : 'text-gray-800'}`}>
                    {format(day, 'd')}
                  </div>
                  {isToday && !isSelected && <div className="w-1 h-1 bg-[#4A9FD8] rounded-full mx-auto mt-1"></div>}
                </button>
              );
            })}
          </div>
        ...
        {selectedDate && (
          <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-4 border border-white/40">
              ...
              {getAvailableSlots(selectedDate).map(time => (
                <button
                  ...
                  className={`p-3 rounded-xl text-sm font-medium transition-all ${
                    selectedTime === time ? 'bg-gradient-to-br from-[#4A9FD8] to-[#2D88C4] text-white shadow-lg' :
            ...
            {getAvailableSlots(selectedDate).length === 0 && (
              <p className="text-sm text-gray-500 text-center py-4">Không có khung giờ trống trong ngày này</p>
            )}
          </div>
        )}
              ...
              {appointmentTypes.map(type => (
                <button
                  ...
                  className={`p-3 rounded-xl text-sm font-medium transition-all text-left ${
                    selectedType === type.value ? 'bg-gradient-to-br from-[#4A9FD8] to-[#2D88C4] text-white shadow-lg' :
                    'bg-white/60 hover:bg-white/80 text-gray-700'
                  }`}
                >
                  {type.label}
